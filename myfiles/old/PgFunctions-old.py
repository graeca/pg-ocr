...
def cutPunctuation(img):
    ...
    for i in  range(0, len-1):
        ...
        if rev_index[i+1]<rev_index[i]-1: #by pass the contigues zeros that exist
            ...
            for j in range(rev_index[i+1],rev_index[i]+1):
               # print(j,"+")
               # print(y[j])
               #normalize
               
               sum_ink=sum_ink+y[j]/(width*height)
               #sum_ink=sum_ink+y[j]
               
               
            #width=np.size(img,1)
            #print(sum_ink,"sum_ink")
            xc=rev_index[i+1]
            sum_ink=sum_ink*1000
            #print(sum_ink,"sum_ink")
            
            if sum_ink < 35:
                #width=np.size(img,1)
                img = img[0:width, 0:xc]
                img=255*img
                img=abs(255-img)
                imgP=img
                ###print(img)
                #img = 255*cv2.bitwise_not(img)
                ###plt.figure() 
                ###print(":::::::::::")
    ...
    for (i, c) in enumerate(contours):
       ...
       #cv2.imwrite('test/'+str(i)+'test_er.png',crop_img_erosion)
        ...
        if res==True:
            ...
            crop_img = img[y-10:y+h, x:x+w]
            #cv2.imwrite('test/'+ folder +'/'+str(i)+'test.png',crop_img)

        
        clean_img=clearBorder(crop_img)
        clean_img=cutPunctuation(clean_img)
        #clean_img=clearBorder(clean_img.astype(int))
        wordname=str(y)+'-'+str(x)+'-'+str(w)+'-'+str(h)
...
def xxxsearchData(query,folder):
...
def test2(query,file):
    network1 = []
    network2 = []
    for line in range(5):
        network1.append(line)
        line=line+1
        network2.append(line)
